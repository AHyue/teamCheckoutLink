...
一个无需服务器的浏览器扩展。访问 ChatGPT 官网时，它会显示可拖动的右侧悬浮面板，使用当前登录会话或用户手动提供的 Session access token 提交团队结账参数，并显示官方托管结账链接。
...
2. 开启“开发者模式”。
3. 点击“加载已解压的扩展程序”。
4. 选择本项目目录 `D:\team-long-link`。

Edge 也可以直接选择打包后的 `D:\team-long-link\dist\edge` 目录；ZIP 文件为 `dist\team-long-link-edge.zip`。
...
2. 页面右侧会自动显示悬浮工具；拖动顶部可移动位置，点击 `−` 可收起，点击 `×` 可关闭。位置、收起和关闭状态都会保存。
...
5. 点击“各地区 2 席优惠参考”会在悬浮工具左侧展开同高的参考栏，拖动主面板时会一起移动，右侧页面仍可操作。价格按折合人民币由低到高排列；首次打开时读取 ChatGPT 官方地区价格配置和最新汇率，之后使用当前页面内存缓存，需要更新时点击“刷新价格”。点击参考表中的地区可切换国家。
...
- 自动读取或手动粘贴的 `accessToken` 只保留在当前 ChatGPT 页面内存中，不会发送给扩展弹窗，也不会写入扩展存储。关闭手动模式或悬浮面板会清除手动输入。
- 扩展只保存工作区、席位、地区、货币和优惠码等表单偏好。
- 地区参考表从 `chatgpt.com/backend-api/checkout_pricing_config/configs/{国家代码}` 读取 ChatGPT 官方价格配置，并从 `open.er-api.com` 读取公开汇率；首次打开后缓存在当前页面内存中，仅在用户点击“刷新价格”时重新请求，不写入扩展存储。
...
- 使用的是 ChatGPT 网页内部接口，接口发生变化时可能需要更新扩展。
