...
- **Mozilla Firefox**：1.0.0 已在 Firefox Add-ons 公开发布，可[安装 Team Checkout Link](https://addons.mozilla.org/addon/team-checkout-link/)。
...
5. 点击“各地区 2 席优惠参考”会在悬浮工具左侧展开同高的参考栏，拖动主面板时会一起移动，右侧页面仍可操作。价格按折合人民币由低到高排列；汇率在扩展本地缓存 24 小时，到期后下次打开参考表时自动更新。用户也可手动刷新汇率，每滚动 1 小时最多 3 次；请求失败后至少间隔 1 小时才重试。参考表提供 ExchangeRate-API 署名链接。点击参考表中的地区可切换国家。
...
- 扩展在浏览器本地保存工作区、席位、地区、货币和优惠码等表单偏好；汇率缓存也保存在本地，不包含用户填写的 Session 或表单信息。
- 地区参考表从官网同源价格配置地址读取数据，并从 `open.er-api.com` 读取公开汇率；地区价格配置缓存在当前页面内存，汇率数据缓存在扩展本地存储 24 小时，到期后用户下次打开参考表时自动更新。手动刷新每滚动 1 小时最多 3 次；请求失败后至少间隔 1 小时才重试。界面提供 ExchangeRate-API 署名链接。
